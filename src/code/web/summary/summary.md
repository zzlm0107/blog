...
目前大模型的输出都是流式的，如果等其全部返回完再显示，就会等待很长时间，影响用户体验，所以我们需要获取到流式片段，然后解析输出到页面上，获取一个直观的感受体验。

目前常见的解决方案就是基于SSE（Server-Sent Events）或者是WebSocket 协议来实现的：

- SSE是基于HTTP的，且浏览器端可以通过 EventSource API 很方便的处理；当然也可以通过 Fetch + ReadableStream 去手动获取到流片段处理；
- WebSocket 是双向通信，基于WS协议，实现上较为复杂，还需要进行心跳机制，断线重连等额外处理；

这里主要介绍下 SSE 格式的前后端处理逻辑,前端使用浏览器自带的 EventSource API：

1. 后端方面，需要指定 Content-Type 为 `text/event-stream `,并且注意SSE数据格式为 `data: xxxx\n\n`，如果表示发送完毕还需要补充 `data: [DONE]\n\n`，默认发送SSE数据会触发 EventSource 的 message 事件，如果我们向传递其他的事件类型和数据，只需要按格式写入 `event: session-id\n` 表示事件名，紧接着写入对应的data数据，即可完成一次自定义事件与数据的发送。

```js
/* SSE格式返回，顺便通过 SessionId 实现了会话记录的缓存，从而完成大模型上下文记忆功能 */
...
2. 前端方面，通过创建 EventSource 实例，连接到对应api地址（只支持get请求），实例会自动解析SSE格式的数据，其event.data 就是数据部分，持续获取拼接后就可以由专门的markdown解析器完成内容展示。
...
```js
  ...
  content.addEventListener('scroll', () => {
    const threshold = 20;
    const isAtBottom = (content.scrollHeight - content.scrollTop - content.clientHeight) <= threshold;
...
3. 补充fetch + ReadableStream 处理：由于 EventSource 只支持 get请求，无法携带复杂的请求头或 body，且无法处理二进制数据，所以必要时我们需要手动解析流数据
