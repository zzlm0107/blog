...
操作完成后，会在项目根目录下会生成`.eslintrc.cjs` 配置文件。但是我们发现似乎依然有一些问题(也可能不会出现)：
...
4. 如果我们想在保存时自动修复eslint的错误，可以在项目根目录下创建一个.vscode文件夹然后创建settings.json中添加如下配置(需要eslint插件):
...
安装成功后，项目根目录创建`.prettierrc.cjs`，并配置相关代码格式。
...
*注：单独配置prettier可能会不生效（具体原因未知），可以尝试重启或继续下面的配置，将prettier的规则添加到eslint中，让eslint进行检测。*

*注：虽然我们在这里配置了换行符为lf，但vscode有默认的换行符配置，这样可能会出现冲突，建议在vscode的设置里搜索eol（end of line）将默认换行符也修改为lf。*
...
注意此时可能整个文件都会飘红，这是因为 `eslint-plugin-prettier` 将 Prettier 的规则设置到 ESLint 的规则中，导致对cjs文件的校验规则变化了，例如要使用单引号等，我们只需要右键将 Prettier 设置为默认格式化工具，再格式化一次即可修复，此时整个代码格式化就都配置完毕了,如不生效可尝试重启vscode。
...
然后就可以向 `.commitlintrc.cjs` 配置文件添加自动流程信息了，可以参考[cz-git](https://cz-git.qbb.sh/zh/config/) 的配置模板。之后每次提交，我们在终端运行 `yarn commit` 即自动执行`git add .` 并开启提交流程。
...
css: {
  ...
  preprocessorOptions: {
    ...
    scss: {
      additionalData: `@use "src/styles/globalVar.scss" as *; @use "src/styles/globalMixin.scss" as *;`
    }
  }
},
```
...
再更新`eslintrc` ，重启项目生成配置文件即可解决,或者重启vscode再启动项目查看。
...
```vue
<template>
  <svg aria-hidden="true" class="svg" :style="{ width, height }">
