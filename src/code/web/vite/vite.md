...
vite的核心包括两部分，esbuild实现开发环境极快的启动和热更新，rollup实现打包的兼容性处理。

**开发启动流程**

1. Vite会启动一个**开发服务器**和一个**依赖预构建服务器**
2. 依赖预构建服务启动时，Vite会使用极快的用 **esbuild** 扫描并预打包你的第三方依赖，将它们都转为原生的ESM后合并依赖，减少浏览器在开发时需要发起的请求数；且预构建后的依赖会被缓存，进一步提升后续的启动速度。
3. 开发服务器启动后，并不会全量构建项目，而是把浏览器视作一个ES模块的运行环境，实现按需编译。即只有当浏览器请求某个文件的时候，才会实时的编译返回。
4. 当修改某一个文件的时候，Vite会让浏览器重新请求并替换被修改的那个模块，无需刷新整个页面。（过程通过WebSocket保持通信）

**打包构建流程**

1. 解析命令行参数并读取根目录下的配置文件 vite.config.js ， 然后进行配置合并，确定最终的构建配置对象（包含口文件、输出目录、插件列表、Rollup 选项、环境变量模式等）。

2. 依赖预构建，因为 Rollup 打包时需要处理的是标准的 ESM 模块，通过esbuild的预构建，将第三方依赖都转为ESM并合并缓存，提供给 Rollup 使用。

3. 加载环境变量文件，确保构建过程中可以访问到正确的环境变量。

4. Rollup构建：

   4.1  确定入口文件，加载所有 Vite 插件，以及用户配置的 Rollup 插件；

   4.2  从入口开始，递归解析所有 `import` 语句，构建模块依赖图；遇到第三方依赖，使用预构建后的产物，遇到 .vue 	等非标准文件时，通过 Vite 插件进行代码转换。

   4.3  利用 Babel 进行代码转化与降级

   4.4  Rollup 会静态分析模块的导入导出，移除未被使用的代码，进行 Tree Shaking

5. 代码分割：根据配置和动态导入，Rollup 将代码拆分为多个 chunk（块）

6. 静态资源处理：小于 `assetsInlineLimit`（默认 4KB）的资源会被转换为 Base64 字符串直接嵌入 JS/CSS，减少 HTTP 请求。大于阈值的资源会文件名中加上**内容哈希值**被复制到输出目录。`public` 下的文件会被原样复制到输出目录根路径，不经过哈希处理。

7. 代码压缩与混淆：使用 **esbuild** 进行代码压缩（minify），移除空格注释，缩短变量名等，进一步减小文件体积。

8. Rollup 生成最终的文件内容，并根据 `output` 配置写入磁盘，默认输出到 `dist` 目录。
